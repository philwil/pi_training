...
void loop() 
{
  ...
  red = myFavoriteColors[c][0];
  green = myFavoriteColors[c][1];
  blue = myFavoriteColors[c][2]; 
  ...
  moveit = 0;
  // are we moving or have we timed out
  if (abs(newVector - storedVector) > MOVE_THRESHOLD)
    moveit = 1;
 // if ( millis() > (flash_time+10000))
 //   moveit =1;
    //|| (millis()<flash_time)) {
    if(moveit == 1)
    {
     ...
     else
     {
      ...
      strip.setPixelColor(last_led, strip.Color(0, 0, 0));
      strip.setPixelColor(next_led, strip.Color(red, green, blue));
      strip.show();
      last_led = next_led;
