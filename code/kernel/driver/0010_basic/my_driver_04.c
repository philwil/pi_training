...
/*  
 *  my_driver_04.c - Basic driver training
 * add ioctl
 * add poll
 */
...
static DECLARE_WAIT_QUEUE_HEAD(driver_poll_wait);
static int driver_state;
static inline void driver_wakeup_readers(void);
...
{
  driver_state++;
  wake_up(&driver_poll_wait);
}

static unsigned int driver_poll(struct file *filp, poll_table *wait)
{
    poll_wait(filp, &driver_poll_wait, wait);
    if (filp->f_version != driver_state) 
      {
	filp->f_version = driver_state;
